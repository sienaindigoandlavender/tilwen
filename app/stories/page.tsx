...
export const metadata = {
  title: 'Stories — Moroccan & Amazigh Material Culture',
  description: 'Short stories on Moroccan and Amazigh material culture — natural dye, weaving, motifs, and the life of a rug.',
  alternates: { canonical: 'https://tilwen.com/stories' },
  openGraph: { title: 'Stories — Moroccan & Amazigh Material Culture', description: 'Short stories on Moroccan and Amazigh material culture.', url: 'https://tilwen.com/stories' },
}
        ...
        <div className="jl-top">
          <div className="container">
            <div className="jl-top__inner">
              <span className="jl-top__title">Stories</span>
              <span className="jl-top__count">
                {stories.length} {stories.length === 1 ? 'essay' : 'essays'}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="container" style={{ display: 'flex', gap: 'var(--sp-12)', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            <div className="jl-filter-group">
              ...
              <div className="jl-filter-tags">
                {Array.from(new Set(stories.flatMap(e => e.theme_tags))).map(tag => (
            ...
            <div className="jl-filter-group">
              ...
              <div className="jl-filter-tags">
                {Array.from(new Set(stories.flatMap(e => e.region_slugs))).map(r => (
        ...
        <div className="container">
          <div className="jl-grid">
            {stories.map((essay, i) => {
              ...
              const year = essay.published_at ? new Date(essay.published_at).getFullYear() : new Date().getFullYear()
              const code = `(${String(i).padStart(3,'0')}-${String(year).slice(2)})`
              return (
                <Link key={essay.slug} href={`/stories/${essay.slug}`} className="jl-item">
          ...
          <div className="jl-statement">
            <p className="jl-statement__text">
              Not a feed. Each story is a permanent object, written to be read once, not scanned for updates. The archive accumulates; it does not flow.
            </p>
          </div>
        </div>
      </div>
