...
export async function generateStaticParams() {
  ...
  return stories.map(e => ({ slug: e.slug }))
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const essay = await getStoryBySlug(params.slug)
  if (!essay) return {}
  return {
    title: essay.title,
    description: essay.excerpt,
    alternates: { canonical: `https://tilwen.com/stories/${essay.slug}` },
    openGraph: { title: essay.title, description: essay.excerpt, type: 'article', url: `https://tilwen.com/stories/${essay.slug}`, images: essay.cover_image ? [{ url: essay.cover_image, alt: essay.title }] : [] },
  }
}
      ...
      <article className="ep magazine-surface">
        ...
        <div className="container">
          ...
          <div className="ep-nav">
            <Link href="/stories" className="ep-back">← Stories</Link>
          ...
          {relatedEssays.length > 0 && (
            <div className="ep-related">
              ...
              <div className="ep-related-grid">
                {relatedEssays.map(e => (
                  <Link key={e.slug} href={`/stories/${e.slug}`} className="ep-related-item">
